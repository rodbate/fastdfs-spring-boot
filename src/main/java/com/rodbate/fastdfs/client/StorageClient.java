...
import com.rodbate.fastdfs.client.exchange.StreamReplier;
...
final class StorageClient {
     ...
     * @return 下载内容长度
     */
    CompletableFuture<Long> download(StorageServer server, FileId fileId, Object output) {
        return executor.execute(
                server.toInetAddress(),
     ...
     * @return 下载进度
     */
    CompletableFuture<Long> download(StorageServer server, FileId fileId, Object output, int offset, int size) {
        return executor.execute(
                server.toInetAddress(),
